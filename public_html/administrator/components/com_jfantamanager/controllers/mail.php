...
//        exit;
        ...
        $query      = mysql_query("SELECT MIN(`giornata`) as giornata FROM `#__fanta_calendario` WHERE `data` >= '$data_fine'");
        $giornata       = mysql_fetch_object($query)->giornata;
        
        $result = mysql_query("SELECT name,email,#__fanta_squadra.id,nome,data,ora
                FROM `#__fanta_squadra`
                LEFT JOIN #__fanta_voti_squadra ON (#__fanta_squadra.id = #__fanta_voti_squadra.squadra_id)
                LEFT JOIN #__users ON (#__fanta_squadra.created_by = #__users.id)
        ...
        $classifica = mysql_query('SELECT nome, SUM( #__fanta_voti_squadra.punti ) AS punti
                        FROM #__fanta_squadra
                        LEFT JOIN #__fanta_voti_squadra ON ( #__fanta_squadra.id = #__fanta_voti_squadra.squadra_id )
                        WHERE lega =1
                        GROUP BY `id`
