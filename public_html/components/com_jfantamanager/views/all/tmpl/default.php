    ...
    foreach ($this->squadre as $i => $squadra)
    {
        if(count($this->giocatori[$i])>0):
            ...
            {   
				...
				if(($this->ufficiale) && $this->giocatori[$i]->stato[$j]=='in' && $j>12)
					$icon=" <img src='/components/com_jfantamanager/images/entrato_s.png' style='height:12px'>";
				elseif(($this->ufficiale) && $this->giocatori[$i]->stato[$j]=='out' && $j<=12)
					$icon=" <img src='/components/com_jfantamanager/images/uscito_s.png' style='height:12px'>";
				else
					$icon='';
...
//                    echo "<tr $seconda". (($j%2)?"class='bianco'":"")  . (($this->giocatori[$i]->stato[$j]=='' && $this->ufficiale)?"id='left'":"")."><td>$giocatore->pos</td><td id='giocatore_".((!$this->ufficiale)?"":$this->giocatori[$i]->stato[$j])."'>$giocatore->nome</td><td>$giocatore->voto</td><td>".bonus($giocatore)."</td><td></td><td>$giocatore->totale</td></tr>";
              echo "<tr $seconda". (($j%2)?"class='bianco'":"")  . (($this->giocatori[$i]->stato[$j]=='' && $this->ufficiale)?"id='left'":"").">
			  <td><img src='/components/com_jfantamanager/images/".$giocatore->pos.".png'></td>
			  <td id='giocatore_".((!$this->ufficiale)?"":$this->giocatori[$i]->stato[$j])."'>".substr($giocatore->nome,0,11).$icon."</td>
			  <td>$giocatore->voto</td>
			  <td>".bonus($giocatore,$this->root)."</td>
			  <td>".malus($giocatore,$this->root)."</td>
			  <td>$giocatore->totale</td>
			  </tr>";
            ...
            if($this->ufficiale)
				...
				echo "<div class='result'><span id='goal'></span>".$this->giocatori[$i]->punti."</div>";
            echo "</div></div>";
        endif;
    }
    ?>
